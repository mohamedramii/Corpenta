...
export const Card = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { customClass?: string }>(
  ({ customClass, ...rest }, ref) => (
    <div 
      ref={ref} 
      {...rest} 
      className={`absolute top-1/2 left-1/2 rounded-2xl border border-white/20 bg-white shadow-2xl transform-gpu will-change-transform ${customClass ?? ''} ${rest.className ?? ''}`.trim()} 
    />
  )
)
